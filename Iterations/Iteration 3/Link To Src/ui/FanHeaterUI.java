...
public class FanHeaterUI {
    ...
    private boolean on; // disable everything except on/off button when turned off

    // Screen 1 UI components
    ...
    private final int PRESET_HOUR = 12;
    private final int PRESET_MINUTE = 0;
    private final double PRESET_TEMP = 15.0;
    ...
    private void initState() {
        ...
        //Screen 3
        newTimerSelection = 1;
        newTimerHour = PRESET_HOUR;
        newTimerMinute = PRESET_MINUTE;
        newTimerTargetTemperature = PRESET_TEMP;
    ...
    private void buildScreen2() {
        ...
        if (componentsManager.getTimerEntry(currentTimerCount).length == 0) {
            ...
            timerTime.setText("EE:EE Uhr");
            timerTemp.setText("EE °C");
        } else {
        ...
        saveTimerBackButton.addActionListener(d -> {
            ...
            if (saveSelection == 1) {
                ...
                if (!timeAlreadyTaken) {
                    ...
                    currentTimerCount = componentsManager.getTimerCount(); // neu angelegten Timer direkt anzeigen
                    frame.remove(saveTimerButtonPanel);
                    frame.remove(savePanel);
                    newTimerHour = PRESET_HOUR;
                    newTimerMinute = PRESET_MINUTE;
                    newTimerTargetTemperature = PRESET_TEMP;
                    saveSelection = 1;
                    buildScreen2(); // Screen 2 neu aufbauen mit aktualisierten Daten
                }
    ...
    private void buildSimulationFrame() {
        ...
        simulateFrame.setLayout(new GridLayout(4, 1, 10, 10));
        ...
        targetTemp = new JLabel("Eingestellte Temp: " + componentsManager.getTargetTemperature() + " °C", SwingConstants.CENTER);
        targetTemp.setFont(textFont);

        simulateFrame.add(openWindowButton);
        simulateFrame.add(timeLabel);
        simulateFrame.add(statusLabel);
        simulateFrame.add(targetTemp);
    ...
    private Timer createSimUpdateTimer(){
        return new Timer(1000, f -> {
            ...
            targetTemp.setText("Eingestellte Temp: " + componentsManager.getTargetTemperature() + " °C");
            if (componentsManager.isWindowOpen()){
